...
const BasicDetailsForm: React.FC<BasicDetailsFormProps> = ({
  ...
  useEffect(() => {
    setIsValid(!!carDetails.title && !!carDetails.location && !!carDetails.bodyType);
  }, [carDetails.title, carDetails.location, carDetails.bodyType]);
        ...
        <div>
          ...
          <div className="relative mb-3">
            ...
            <select
              ...
              onChange={(e) => updateCarDetails({ bodyType: e.target.value })}
              className="block w-full pl-10 pr-3 py-3 border border-gray-300 rounded-lg shadow-sm 
          ...
          <div className="flex flex-wrap gap-2">
            {bodyTypes.slice(0, 4).map((type) => (
              <button
                key={type}
                type="button"
                onClick={() => updateCarDetails({ bodyType: type })}
                className={`px-3 py-1.5 text-sm rounded-full transition-colors ${
                  carDetails.bodyType === type
